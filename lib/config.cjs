...
function loadEnvFile() {
  ...
  try {
    ...
    console.error(`[pi-web-voice] cannot read ${ENV_FILE}: ${errorMessage(error)}`);
  }
  return fileEnv;
}
...
// Ten minutes of the browser's 16 kHz mono PCM is about 19.2 MB. Keep the
// upload ceiling at the Azure OpenAI file limit, and give a full-length take
// enough time to finish transcribing after the upload completes.

 ...
 *
 * Keys normally live in ~/.pi/agent/voice.env rather than a shell profile.
 * Anything already exported takes precedence over that file.
 */

const ENV_FILE = process.env.PI_VOICE_ENV || path.join(os.homedir(), ".pi", "agent", "voice.env");
...
const PROJECT_SESSIONS = 5; // past sessions consulted under `project` scope
...
/** Accepts a bare resource name or a full https URL. */
...
function loadConfig() {
  ...
  return {
    ...
    // The vocabulary sent with each request is mined from the conversation;
    // see lib/context.cjs. This only chooses how wide to look.
    context: {
      scope: text(env.PI_VOICE_CONTEXT, "project"), // project | session | off
      maxTerms: MAX_TERMS,
      bytes: CONTEXT_BYTES,
    ...
    ui: {
      mode: text(env.PI_VOICE_MODE, "toggle"), // toggle | hold
      autoSend: bool(env.PI_VOICE_AUTO_SEND, false),
    },
    ...
    azureSpeech: {
      ...
      style: text(env.PI_VOICE_STYLE, "clean"), // clean | verbatim
    },
    ...
    azureOpenAI: {
      ...
      deployment: text(env.PI_VOICE_DEPLOYMENT, "gpt-4o-transcribe"),

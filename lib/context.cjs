...
/**
 * Returns the ranked vocabulary for a request: pinned terms first, then terms
 * mined from the conversation, capped at `limit`.
 *
...
function collectTerms(sessionId, cwd, config, limit) {
  const pinned = config.terms.filter(acceptable);
  if (config.context.scope === "off") return pinned.slice(0, limit);
  ...
  if (cached && cached.expires > now) {
    ...
    if (!found) return pinned.slice(0, limit);
    ...
    mined = [...scores.entries()].sort((a, b) => b[1] - a[1]).map(([term]) => term);
    cache.set(key, { expires: now + CACHE_TTL_MS, terms: mined });
  }

  const seen = new Set(pinned.map((term) => term.toLowerCase()));
  const merged = [...pinned];

...
/** @typedef {{ file: string | null, dir: string }} SessionLocation */
...
/** @type {Map<string, { expires: number, terms: string[] }>} */
const cache = new Map();
...
/**
 * Locate the JSONL file for a session id and its project.
 * @param {string | null | undefined} sessionId
 * @returns {SessionLocation | null}
 */
function resolveSession(sessionId) {
  ...
  for (const project of projects) {
    ...
    const match = files.find((name) => {
      ...
      return separator > 0 && name.slice(separator + 1) === `${sessionId}.jsonl`;
    });
 ...
 */
/** @type {{ expires: number, byCwd: Map<string, string> }} */
    ...
    for (const project of projects) {
      ...
      try {
        const newest = fs
          ...
          .pop();
        if (!newest) continue;
        const recorded = readHeaderCwd(path.join(dir, newest));
  ...
  try {
    ...
    try {
      while (length < buffer.length) {
        const count = fs.readSync(handle, buffer, length, Math.min(4096, buffer.length - length), length);
        if (count === 0) break;
        const newline = buffer.indexOf(0x0a, length);
        length += count;
    ...
    return isRecord(header) && typeof header.cwd === "string" ? header.cwd : null;
  } catch {
    return null;
  }
}
...
/**
 * Read the last `maxBytes` of a file and drop the partial first line.
 * @param {string} file
...
/**
 * Score user/assistant prose from one session file.
 * @param {string} file
 * @param {number} weight
 * @param {Map<string, number>} scores
 * @param {number} maxBytes
...
function scoreSession(file, weight, scores, maxBytes) {
  /** @type {Array<{ role: "user" | "assistant", texts: string[] }>} */
  const messages = [];
    ...
    const texts =
      typeof content === "string"
        ...
        : Array.isArray(content)
          ? content.flatMap(/** @param {unknown} block */ (block) =>

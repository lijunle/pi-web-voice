...
function createRouter(config) {
  return async function handleRoute(req, res) {
    ...
    if (route === "/health") {
      json(res, 200, { ok: true, provider: config.provider, context: config.context.scope });
      return;
    }
    ...
    if (route === "/terms") {
      ...
      json(res, 200, { sessionId, cwd, scope: config.context.scope, count: terms.length, terms });
      return;
    }

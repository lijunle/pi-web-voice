...
async function azureSpeech(audio, config, terms) {
  ...
  if (terms.length > 0) definition.phraseList = { phrases: terms };
...
async function azureOpenAI(audio, config, terms) {
...
async function mock(audio, config, terms) {
  ...
  return `[pi-web-voice mock] received ${audio.length} bytes (~${seconds}s of audio)${vocabulary}`;
}

// How many vocabulary entries each backend can usefully absorb.
// Azure phrase lists are documented up to 500; whisper-style prompts are
// capped by tokens, so they get a much smaller slice.
const TERM_BUDGET = {
  "azure-speech": 500,
  "azure-openai": 60,
  openai: 60,
...
async function transcribe(audio, config, terms = []) {
  const provider = PROVIDERS[config.provider];
  if (!provider) throw new Error(`unknown provider: ${config.provider}`);
  return provider(audio, config, terms);
}

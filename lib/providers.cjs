...
/**
 * Azure OpenAI. Two request shapes, chosen by the deployment name:
 *
 * - `gpt-transcribe` speaks the v1 surface and takes structured `keywords[]`
 *   and `languages[]`, which is a better fit than stuffing a term list into a
 *   free-form prompt: no token budget to fight, and nothing the model could
 *   mistake for an instruction.
 * - `gpt-4o-transcribe` and whisper take a `prompt` on the classic
 *   deployment path. That model retires on 15 October 2026.
 */
async function azureOpenAI(audio, config, terms, languages) {
  ...
  const structured = /gpt-transcribe/i.test(deployment);
  ...
  let url;
  if (structured) {
    url = `${endpoint}/openai/v1/audio/transcriptions?api-version=preview`;
    ...
    url = `${endpoint}/openai/deployments/${encodeURIComponent(deployment)}/audio/transcriptions?api-version=${encodeURIComponent(apiVersion)}`;

...
/**
 * Describe the request's VAD setting separately from the service's detection result.
 * @param {VoiceConfig} config
 ...
 * retaining the dictation guidance and automatic VAD. Other Azure model
 * branches receive only a bounded vocabulary prompt.
 * @param {Buffer} audio
 * @param {VoiceConfig} config
 * @param {string[]} terms
...
async function azureOpenAI(audio, config, terms, languages) {
  ...
  if (!response.ok && response.status === 400 && structured) {
    console.error(
      `[pi-web-voice] structured request rejected (400), retrying with vocabulary in the prompt · vad=${vad}`,
    );
